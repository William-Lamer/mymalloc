...
An implicit free list requires traversing every block (both allocated and free) to find a suitable block. On the other hand, an explicit list only visits free blocks. Under many allocation/free operations this is a significant improvement (roughly 5x faster from a specific benchmarj result). This is worth it, evem if it comes at the cost of 16 extra bytes per free block for the `NEXT` and `FREE` pointers. 


**Why a footer boundary tag?**

Coalescing requires knowing wheter the previous block is free or not. The footer allows us to perform backward traversal through the heap in O(1) instead having to walk the entire heap. This is worth it, even at a cost of 8 bytes per block. 


**Why `sbrk` for heap extension?**

`sbrk` extends a single contiguous heap region, which maps very cleanly unto the structure of our heap. I plan in the future to supplement our existing `sbrk` with `mmap` for large allocations (>128KB) like modern memory allocators. 
...
- **Coalescing:** Three adjacent freed blocks merge into one, able to statisfy an allocation request of their combined size without requesting memory from the OS
...
We can run python scripts, and even do JSON processing!
...
- **No mmap for large allocations:** Currently all allocations go through `sbrk`. Using `mmap` for larger allocations would allow large blocks to be returned bacl to the OS rather then be held in the heap.
- **No segregated free list:** Currently a single free list is scanned linealy. Using multiple free lists for different block sizes would greatly reduce search time under cases with high fragmentation.
